...
class CameraManager(object):
    ...
    def render(self, display):
        if self.surface is not None:
            ...
            img_bgr = cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE)#fix to rotate image
            resized_frame = cv2.resize(img_bgr, (900,600))
...
def game_loop(args):
    ...
    try:
        ...
        ###
        x_values = []  # Initialize empty list
        ###
        ...
        while True:
            ...
            if elapsed_time >= 1.0:
                x_values.append(world.player.get_transform().location.x)
                last_time = current_time

                # Create plot of x values
                # plt.plot(x_values)
